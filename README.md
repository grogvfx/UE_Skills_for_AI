...
**Deferred tool loading** is enabled by default. Instead of exposing all tool schemas upfront, the MCP server provides three lightweight tools — `list_toolsets`, `describe_toolset`, and `load_toolset` — so the LLM discovers and loads toolsets on demand, keeping context window usage low. This can be toggled with the `ModelContextProtocol.DeferredToolLoading` console variable.
