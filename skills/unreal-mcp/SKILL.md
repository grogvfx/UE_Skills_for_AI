...
The `.mcp.json` file tells Claude Code where the MCP server is. It is normally generated by the editor at startup, but Claude Code can create it directly if the editor is not yet running. Write this to `.mcp.json` in the project root (the directory containing the `.uproject` file):
