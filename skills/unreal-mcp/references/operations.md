...
| Editor logs "Failed to listen on port"                | Another process holds the default port. Change `ServerPortNumber` in `Config/DefaultEditorPerProjectUserSettings.ini` (or pass `-ModelContextProtocolPort=<port>` on the next launch), restart the editor, and re-run `ModelContextProtocol.GenerateClientConfig ClaudeCode` to refresh `.mcp.json`. |
